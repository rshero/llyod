...
@app.on(events.NewMessage(pattern="^[/!]([Nn][Oo][Vv][Ee][Ll](@LlyodFronteraBot)?)\s.+"))
...
async def novel_details(event: CallbackQuery.Event):
	...
	if data[1] == 'novels':
		...
		try:
			...
			pub_date = 'N/A'
		genres = ', '.join([genre['genre'].capitalize() for genre in series['genres']]) or 'N/A'
		try:
		...
		await event.edit(msg, parse_mode='html', buttons=[[Button.url("Info", url=url)], [Button.inline("❮", data=f"{sid}_novelb_{query}")]])
	...
	if data[1] == 'novelb':
		...
		for c, result in enumerate(results):
			...
			title = result['match'][0][1]
			sid = result['sid']
			buttons.append([Button.inline(text=title, data=f"{sid}_novels_{queries.index(query)}")])
