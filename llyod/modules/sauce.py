...
from pysaucenao import SauceNao, PixivSource, AnimeSource, MangaSource, VideoSource, BooruSource

saucenao = SauceNao(api_key=SAUCE_KEY, min_similarity=60, results_limit=2)
media_types =( '.jpeg', '.jpg', '.png', '.gif', '.webp')


@app.on(events.NewMessage(pattern="^[/!][Ss][Aa][Uu][Cc][Ee](@LlyodFronteraBot)?(\s.*)?"))
            ...
            if isinstance(result, PixivSource):
                ...
                if urls := result.urls:
                    for url in urls:
                        ...
                        name = names[1] if len(names) > 2 and names[0] == "www" else names[0]
