...
async def mangadex_search(event: Message):
    ...
    for result in range(btns):
        ...
        buttons.append([Button.inline(text=title, data=f"{results[result]['id']}_dexs_{query}")])
...
async def manga_detail(event: CallbackQuery.Event):
    ...
    if data[1] == 'dexs':
        ...
        if len(desc) > 500:
            desc = desc[:500] + "..."
        genres_list = [tag['attributes']['name']['en'] for tag in series['attributes']['tags']] or "N/A"
        ...
        await event.edit(msg, parse_mode='html', buttons=[[Button.url("Info", url=url)], [Button.inline("❮", data=f"{query}_dexb")]])
    ...
    if data[1] == 'dexb':
        ...
        for result in range(btns):
            ...
            if "ja-ro" in what_title:
                title = unescape(what_title['ja-ro'])
            buttons.append([Button.inline(text=title, data=f"{results[result]['id']}_dexs_{queries.index(query)}")])
...
async def manga_spier(event: Message):
    ...
    if match:
        ...
        if links:
            if "al" in links:
                btns.append(Button.url("AniList",url=f"https://anilist.co/manga/{series['attributes']['links']['al']}"))
            if "mal" in links:
                btns.append(Button.url("MyAnimeList", url=f"https://myanimelist.net/manga/{series['attributes']['links']['mal']}"))
            # anilist = f"https://anilist.co/manga/{series['attributes']['links']['al']}"
