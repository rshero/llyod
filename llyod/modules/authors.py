...
async def authors(event: Message):
    ...
    for result in range(btns):
        ...
        buttons.append([Button.inline(text=name, data=f"{results[result]['record']['id']}_authsearch_{query}")])
...
async def author(event: CallbackQuery.Event):
    ...
    if data[1] == 'authsearch':
        ...
        await event.edit(msg, parse_mode='html', buttons=[[Button.url("Info", url=url), Button.inline("List Series", data=f"{id}_listseries_{query}")], [Button.inline("❮", data=f"{query}_authback")]])
    ...
    if data[1] == 'listseries':
        ...
        await event.edit(f"**List of series for this author:**\n\n{unescape(names)}", buttons=Button.inline("❮", data=f"{id}_authsearch_{query}"))
