...
async def manga(event: Message):
    ...
    for result in range(btns):
        ...
        buttons.append([Button.inline(text=title, data=f"{results[result]['record']['series_id']}_search_{query}")])
...
async def manga_detail(event: CallbackQuery.Event):
    ...
    if data[1] == 'search':
        ...
        title = series['title']
        alt_list = [series['associated'][i]['title'] for i in range(len(series['associated']))]
        ...
        genre_list = [series['genres'][i]['genre'] for i in range(len(series['genres']))]
        genres = ", ".join(genre_list) or "N/A"
        latest = series['latest_chapter'] or "N/A"
        auth_list = [series['authors'][i]['name'] for i in range(len(series['authors']))]
        ...
        await event.edit(msg, parse_mode='html', buttons=[[Button.url("Info", url=url), Button.inline("Groups", data=f"{id}_groups_{query}"), Button.inline("Tags", data=f"{id}_tags_{query}")], [Button.inline("Recommendations", data=f"{id}_reccom_{query}")], [Button.inline("❮", data=f"{query}_back")]])
    ...
    if data[1] == 'groups':
        ...
        if reals == "":
            reals = "Nothing"
        await event.edit(f"**Groups related to this series is:-**\n{unescape(names)}\n**Latest release for this series:-**\n{unescape(reals)}", buttons=Button.inline("❮", data=f"{id}_search_{query}"))
    ...
    if data[1] == 'reccom':
        ...
        if recc == "":
            recc = "Nothing"
        await event.edit(f"**Recommendations for {results['title']}:**\n\n{unescape(recc)}", buttons=Button.inline("❮", data=f"{id}_search_{query}"))
    
    if data[1] == 'tags':
        id = data[0]
        query = data[2]
        series = await series_detail(id)
        ct_list = [series['categories'][i]['category'] for i in range(len(series['categories']))]
        categories = ", ".join(ct_list) or "No Tags found"
        categories = categories.replace("/", "")
        await event.edit(f"**Tags for {series['title']}:**\n\n`{categories}`", buttons=Button.inline("❮", data=f"{id}_search_{query}"))
