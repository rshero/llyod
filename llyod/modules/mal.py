...
async def mal_anime_search(event: Message):
    ...
    for result in range(len(results)):
        ...
        buttons.append(
            [
                Button.inline(
                    text=title, data=f"{results[result]['node']['id']}_mala_{query}"
                )
            ]
        )
    x = await event.reply(
        msg, file="https://telegra.ph/file/33067bb12f7165f8654f9.mp4", buttons=buttons
    )
...
async def mal_detail(event: CallbackQuery.Event):
    ...
    if data[1] == "mala":
        ...
        buttons = [
            [
                Button.url("More Info", url=url),
                Button.inline("Recommendations", data=f"{query}_mreco_{id}"),
            ],
            [Button.inline("❮", data=f"{query}_mback")],
        ]
        ...
        for result in range(len(results)):
            ...
            buttons.append(
                [
                    Button.inline(
                        text=title,
                        data=f"{results[result]['node']['id']}_mala_{queries.index(query)}",
                    )
                ]
            )
        x = await event.edit(
            msg,
            file="https://telegra.ph/file/33067bb12f7165f8654f9.mp4",
            buttons=buttons,
        )
    ...
    if data[1] == "mreco":
        ...
        await event.edit(
            recoms, buttons=[Button.inline("❮", data=f"{id}_mala_{query}")]
        )
